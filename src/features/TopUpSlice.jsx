...
export const getTopUps = createAsyncThunk("topUps/getTopUps", async() => {
    const response = await axios.get('http://44.203.0.77/api/v1/products/topup', { headers: authHeader() });
    return response.data.data;
});

export const saveTopUp = createAsyncThunk("topUps/saveTopUp", async ({ name, amount, gross_amount }) => {
  const response = await axios.post('http://44.203.0.77/api/v1/products/topup', {
      name,
      amount,
      gross_amount
  },{ headers: authHeader() });
...
export const updateTopUp = createAsyncThunk("topUps/updateTopUp", async ({ id, name, amount, gross_amount }) => {
  const response = await axios.put(`http://44.203.0.77/api/v1/products/topup/${id}`, {
      name,
      amount,
      gross_amount
  },{ headers: authHeader() });
...
export const deleteTopUp = createAsyncThunk("topUps/deleteTopUp", async (id) => {
    await axios.delete(`http://44.203.0.77/api/v1/products/topup/${id}` , { headers: authHeader()}) ;
    return id;
