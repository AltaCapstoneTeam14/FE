...
export const getCashOut = createAsyncThunk("cashOut/getCashOut", async() => {
    const response = await axios.get('http://44.201.153.46:8081/api/v1/products/cashout', { headers: authHeader() });
    return response.data.data;
});

export const saveCashOut = createAsyncThunk("cashOut/saveCashOut", async ({ name, coin_amount, balance_amount }) => {
  const response = await axios.post('http://44.201.153.46:8081/api/v1/products/cashout', {
      name,
      coin_amount,
      balance_amount
  },{ headers: authHeader() });
...
export const updateCashOut = createAsyncThunk("cashOut/updateCashOut", async ({ id, name, coin_amount, balance_amount }) => {
  const response = await axios.put(`http://44.201.153.46:8081/api/v1/products/cashout/${id}`, {
      name,
      coin_amount,
      balance_amount
  },{ headers: authHeader() });
...
export const deleteCashOut = createAsyncThunk("cashOut/deleteCashOut", async (id) => {
    await axios.delete(`http://44.201.153.46:8081/api/v1/products/cashout/${id}` , { headers: authHeader()}) ;
    return id;
