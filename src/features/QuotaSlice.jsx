...
export const getQuotas = createAsyncThunk("quota/getQuotas", async() => {
    const response = await axios.get('http://44.201.153.46:8081/api-dev/v1/products/quota', { headers: authHeader() });
...
export const saveQuota = createAsyncThunk("quota/saveQuota", async ({ name, description, stock, provider_id, gross_amount, provider_name}) => {
  const response = await axios.post('http://44.201.153.46:8081/api-dev/v1/products/quota', {
      name,
      description,
      stock,
      provider_id,
